...
public class GlowingMushroomCowNeckLayer : PlayerDrawLayer
{
    public static SlowAsset<Texture2D> neckTexture;

    public override void Load()
    {
        neckTexture = new SlowAsset<Texture2D>($"{nameof(BlockVanity)}/Assets/Textures/Items/Vanity/MushroomCow/GlowingMushroomCowHead_Neck");
    }

    public override Position GetDefaultPosition() => new BeforeParent(PlayerDrawLayers.NeckAcc);

    public override bool GetDefaultVisibility(PlayerDrawSet drawInfo) => 
        drawInfo.drawPlayer.head == EquipLoader.GetEquipSlot(Mod, ModContent.GetInstance<GlowingMushroomCowHead>().Name, EquipType.Head) ||
        drawInfo.drawPlayer.head == EquipLoader.GetEquipSlot(Mod, ModContent.GetInstance<CoolGlowingMushroomCowHead>().Name, EquipType.Head);
