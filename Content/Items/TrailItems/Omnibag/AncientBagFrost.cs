    ...
    public class AncientBagFrost : VanityItem, ITrailItem
    {
        ...
        public void UpdateParticleEffect(Player player, int slot, ArmorShaderData dye)
        {
            if (player.velocity.Length() > 2f)
            {
                ...
                {
                    Dust d = Dust.NewDustPerfect(player.MountedCenter + Main.rand.NextVector2Circular(15, 20), DustID.Frost, Main.rand.NextVector2Circular(2, 2) + player.velocity * 0.1f, 0, Color.White, 1.25f);
                    d.noGravity = true;
                    d.shader = dye;
                }
            }
