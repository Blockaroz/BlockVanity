...
public class RenderTargetDrawContent : INeedRenderTargetContent
{
    ...
    public void Request(int width, int height, int identifier, Action<SpriteBatch> draw)
    {
        ...
        {
            _draws.Add(new DrawContent { requested = true, identifier = identifier, width = width, height = height, draw = draw });
        }
        else
        {
    ...
    public Texture2D GetTarget(int identifier) => _draws.First(n => n.identifier == identifier && n.target != null).target;
